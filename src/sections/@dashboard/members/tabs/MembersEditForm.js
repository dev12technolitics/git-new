...
import { putupdatemembers } from '../../../../redux/slices/members';
...
export default function MembersAddForm({ membersData, id }) {
  ...
  const onSubmit = async (data) => {
    if (projectType?.length) {
      setIsLoading(true);      
      const memberDelete = membersData?.association.filter(item => !projectType.includes(item))
      try {
        ...
        formData.set('memberDelete', JSON.stringify(memberDelete));
        ...
        dispatch(putupdatemembers(id, formData, toast, push, reset, setIsLoading));
      } catch (error) {
        console.error(error);
      }
    } else {
                ...
                <Grid item xs={12} md={8}>
                  <Stack spacing={3}>
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} md={6}>
                        <RHFTextField name="scontact_no" label="Secondary Contact No." type="text" />
                      </Grid>
            ...
            <Card sx={{ p: 3, mb: 2 }}>
              <Grid container spacing={3}>
                <Grid item xs={12} md={12}>
                  ...
                  <Stack direction="row" spacing={1.5} sx={{ mt: 3, mb: 1 }} justifyContent="end">
