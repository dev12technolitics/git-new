...
import GroupDetailTableRow from './GroupDetailTableRow';
...
export default function DetailGroupForm({ groupData, isLoading, id, memberData }) {
  ...
  return (
    <>
      <ToastContainer />
      <Card>
        <GroupsTableToolbar filterName={filterName} onFilterName={handleFilterName} id={id} />
            ...
            <Table size={dense ? 'small' : 'medium'}>
              ...
              <TableBody>
                {(isLoading ? [...Array(rowsPerPage)] : dataFiltered)
                  ...
                  .map((row, index) =>
                    row ? (
                      <GroupDetailTableRow key={index} row={row} index={index} onSubmit={() => onSubmit(row?._id)} />
                    ) : (
